...
public class GlobalExceptionHandler {
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[报表模块表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[报表模块表结构未导入][请检查数据库初始化脚本]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[工作流模块表结构未导入][请检查数据库初始化脚本]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号模块表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[微信公众号模块表结构未导入][请检查数据库初始化脚本]");
        }
        // 4. 商城系统
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[商城系统表结构未导入][请检查数据库初始化脚本]");
        }
        // 5. ERP 系统
        if (message.contains("erp_")) {
            log.error("[ERP 系统表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[ERP 系统表结构未导入][请检查数据库初始化脚本]");
        }
        // 6. WMS 仓库管理系统
        if (message.contains("wms_")) {
            log.error("[WMS 仓库管理系统表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[WMS 仓库管理系统表结构未导入][请检查数据库初始化脚本]");
        }
        // 7. CRM 系统
        if (message.contains("crm_")) {
            log.error("[CRM 系统表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[CRM 系统表结构未导入][请检查数据库初始化脚本]");
        }
        // 8. MES 系统
        if (message.contains("mes_")) {
            log.error("[MES 系统表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[MES 系统表结构未导入][请检查数据库初始化脚本]");
        }
        // 9. IM 即时通讯
        if (message.contains("im_")) {
            log.error("[IM 即时通讯表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[IM 即时通讯表结构未导入][请检查数据库初始化脚本]");
        }
        // 10. 支付平台
        if (message.contains("pay_")) {
            log.error("[支付模块表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[支付模块表结构未导入][请检查数据库初始化脚本]");
        }
        // 11. AI 大模型
        if (message.contains("ai_")) {
            log.error("[AI 大模型表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[AI 大模型表结构未导入][请检查数据库初始化脚本]");
        }
        // 12. IoT 物联网
        if (message.contains("iot_")) {
            log.error("[IoT 物联网表结构未导入][请检查数据库初始化脚本]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[IoT 物联网表结构未导入][请检查数据库初始化脚本]");
        }
        return null;
    }
